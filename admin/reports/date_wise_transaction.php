...
<div class="card card-outline card-primary rounded-0 shadow">
	<div class="card-header">
		<h3 class="card-title">Rapport de transaction par date</h3>
					...
					<form action="" id="filter">
						<div class="row align-items-end">
							<div class="form-group col-md-3">
								<label for="from" class="control-label">Date de</label>
                                <input type="date" name="from" id="from" value="<?= $from ?>" class="form-control form-control-sm rounded-0">
							</div>
							<div class="form-group col-md-3">
								<label for="to" class="control-label">Date a</label>
                                <input type="date" name="to" id="to" value="<?= $to ?>" class="form-control form-control-sm rounded-0">
							</div>
							<div class="form-group col-md-4">
                                <button class="btn btn-primary btn-flat btn-sm"><i class="fa fa-filter"></i> Filtrer</button>
			                    <button class="btn btn-sm btn-flat btn-success" type="button" id="print"><i class="fa fa-print"></i> Imprimer</button>
							</div>
						</div>
					</form>
			</fieldset>
        ...
        <div class="container-fluid">
			<div class="row">
				...
				<div class="col-8">
					...
					<h3 class="text-center"><b>Rapport de transaction d'assurance par date</b></h3>
					<h5 class="text-center"><b> à compter de </b></h5>
					<?php if($from == $to): ?>
			...
			<table class="table table-bordered table-hover table-striped">
				...
				<tbody>
					...
					<?php if($qry->num_rows <= 0): ?>
						<tr>
							<th class="py-1 text-center" colspan="8">Pas de données.</th>
						</tr>
					<?php endif; ?>
				</tbody>
			</table>
...
<script>
	$(document).ready(function(){
       ...
       $('#print').click(function(){
		   ...
		   var _h = $('head').clone()
		   var _el = $('<div>')
		   _h.find("title").text("Rapport de transactions par date - Vue avant impression")
