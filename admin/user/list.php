...
<div class="card card-outline card-primary">
	<div class="card-header">
		...
		<div class="card-tools">
			<a href="?page=user/manage_user" class="btn btn-flat btn-primary"><span class="fas fa-plus"></span>  Créer nouveau </a>
		</div>
	</div>
	<div class="card-body">
							...
							<td align="center">
								 <button type="button" class="btn btn-flat btn-default btn-sm dropdown-toggle dropdown-icon" data-toggle="dropdown">
				                  ...
				                  <div class="dropdown-menu" role="menu">
									...
									<?php if($row['status'] != 1): ?>
				                    <a class="dropdown-item verify_user" href="javascript:void(0)" data-id="<?= $row['id'] ?>"  data-name="<?= $row['username'] ?>"><span class="fa fa-check text-primary"></span> Verifier</a>
				                    <div class="dropdown-divider"></div>
									<?php endif; ?>
...
<script>
	$(document).ready(function(){
		$('.delete_data').click(function(){
			_conf("Êtes-vous sûr de supprimer cet utilisateur définitivement?","delete_user",[$(this).attr('data-id')])
		})
		$('.table td,.table th').addClass('py-1 px-2 align-middle')
		$('.table').dataTable();
		$('.verify_user').click(function(){
			_conf("Êtes-vous sûr de vérifier <b>"+$(this).attr('data-name')+"<b/>?","verify_user",[$(this).attr('data-id')])
		})
	})
	function delete_user($id){
		...
		$.ajax({
			...
			error:err=>{
				console.log(err)
				alert_toast("Une erreur sest produite.",'error');
				end_loader();
			},
