...
Class Users extends DBConnection {
	...
	public function save_users(){
		...
		if(isset($_FILES['img']) && $_FILES['img']['tmp_name'] != ''){
			...
			if(!in_array($type,$allowed)){
				$resp['msg'].=" L'image n'a pas pu être téléchargée en raison d'un type de fichier non valide.";
			}else{
				$new_height = 200; 
				$new_width = 200; 
	...
	public function save_employee(){
		...
		if($chk > 0){
			$resp['status'] = 'failed';
			$resp['msg'] = ' Le code d.employé existe déjà.';
		}else{
			foreach($_POST as $k => $v){
			...
			if($save){
				...
				$resp['status'] = 'success';
				if(empty($id))
				$resp['msg'] = ' Employé ajouté avec succès';
				else
				$resp['msg'] = ' Les détails de l.employé ont été enregistrés avec succès';
				if(isset($_FILES['img']) && $_FILES['img']['tmp_name'] != ''){
					...
					if(!in_array($type,$allowed)){
						...
						$resp['msg'].=" L'image n'a pas pu être téléchargée pour une raison inconnue.";
						}
					}
				}
			}
